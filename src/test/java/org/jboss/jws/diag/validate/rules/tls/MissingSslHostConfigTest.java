...
public class MissingSslHostConfigTest {

    private final MissingSslHostConfigRule rule = new MissingSslHostConfigRule();
    ...
    @Test
    void shouldFlagWhenSslHostConfigIsMissing() throws Exception {
        Document serverXml = parseFixture("/fixtures/tls/server-missing-ssl-host-config.xml");
        RuleContext ctx = new RuleContext(Path.of("/dummy"), serverXml, null, "testuser");

        List<Finding> findings = rule.evaluate(ctx);

        assertThat(findings).hasSize(1);
        assertThat(findings.get(0).getRuleId()).isEqualTo(RuleId.TLS_004);
        assertThat(findings.get(0).getSeverity()).isEqualTo(Severity.WARN);
        assertThat(findings.get(0).getDetail()).contains("does not have SSLHostConfig");
    }
